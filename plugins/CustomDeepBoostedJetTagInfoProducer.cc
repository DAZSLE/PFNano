...
void CustomDeepBoostedJetTagInfoProducer::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (std::size_t jet_n = 0; jet_n < jets->size(); jet_n++) {
    ...
    if (fill_vars) {
      //std::cout << "\n filling variables for this jet with pt " << jet.pt() << " eta " << jet.eta() << " phi " << jet.phi() << " mass " << jet.mass() << " jetpx " << jet.px() << " jetpy " << jet.py() << " jetpz " << jet.pz() << std::endl;
...
void CustomDeepBoostedJetTagInfoProducer::fillParticleFeatures(DeepBoostedJetFeatures &fts, const reco::Jet &jet) {
  ...
  //std::cout << " daughters before " << jet.daughterPtrVector().size() << std::endl;
  for (const auto &dau : jet.daughterPtrVector()) {
    ...
    if ((puppiWgt(dau)) < min_puppi_wgt_) {
      //std::cout << " NOT min pup wgt " << std::endl;
      continue;
    }
	...
	//std::cout << " NOT maxsip3dsig " << std::endl;
        continue;
      }
    }
  ...
  //std::cout << " npf cand " << daughters.size() << std::endl;
  ...
  for (const auto &cand : daughters) {
    ...
    if (packed_cand) {
      ...
      float dzsig = packed_cand->bestTrack() ? ip_sign * packed_cand->dz() / packed_cand->dzError() : 0;
      float dxysig = packed_cand->bestTrack() ? ip_sign * packed_cand->dxy() / packed_cand->dxyError() : 0;

      //std::cout << cand.key() << " jet " << jet.pt() << ", pt = " << cand->pt() << ", eta = " << cand->eta() << ", phi = " << cand->phi() << ", mass = " << cand->mass() <<  " ";
      //std::cout << " dz " << ip_sign * packed_cand->dz() << " dxy " << ip_sign * packed_cand->dxy() << " dzsig " << dzsig << " dxysig " << dxysig << std::endl;
      ...
      float quality = packed_cand->bestTrack() ? packed_cand->bestTrack()->qualityMask() : 0;

      //std::cout << " vtxass " << packed_cand->pvAssociationQuality() << " lost " << packed_cand->lostInnerHits() << " quality " << quality << std::endl;
      ...
      //std::cout << " charge " << packed_cand->charge() << " el " << (std::abs(packed_cand->pdgId()) == 11) << " mu " << (std::abs(packed_cand->pdgId()) == 13) << " chad " << (std::abs(packed_cand->pdgId()) == 211) << " gamm  " << (std::abs(packed_cand->pdgId()) == 22) << " nhad " << (std::abs(packed_cand->pdgId()) == 130) << std::endl;
      ...
      //std::cout << " reco cand dz " << dz << " dxy " << dxy << std::endl;
    }
    ...
    //std::cout << " ptP4 " <<  candP4.pt() << " ptnopup " << cand->pt() << std::endl;
    fts.fill("pfcand_mask", 1);
    fts.fill("pfcand_pt_log_nopuppi", std::log(cand->pt()));
    fts.fill("pfcand_e_log_nopuppi", std::log(cand->energy()));
    //std::cout << " pfcand_etarel " << etasign * (candP4.eta() - jet.eta()) << " etasign " << etasign << std::endl;
    //std::cout << " pfcand_phirel " << reco::deltaPhi(candP4, jet) << " pfcand_pt_log_nopuppi " << std::log(cand->pt()) << " pfcand_e_log_nopuppi  " << std::log(cand->energy()) << " pfcand_abseta " << std::abs(candP4.eta()) << std::endl;
    ...
    if (trk) {
      ...
      TrackInfoBuilder trkinfo(track_builder_);
      trkinfo.buildTrackInfo(&(*cand), jet_dir, jet_ref_track_dir, *pv_);
      //std::cout << " normch " << std::floor(trk->normalizedChi2()) << std::endl;
      //std::cout << " btagetarel " << trkinfo.getTrackEtaRel() << " ratio " << trkinfo.getTrackPtRatio() << " ppar " << trkinfo.getTrackPParRatio() << std::endl;
      //std::cout << " 3dval " << ip_sign * trkinfo.getTrackSip3dVal() << " ipsign " << ip_sign << " 3dsig " << ip_sign * trkinfo.getTrackSip3dSig() << " distval " << trkinfo.getTrackJetDistVal() << std::endl;
    ...
    // std::cout << " px " << pfp4.Px() << " py " << pfp4.Py() << " pz " << pfp4.Pz() << " e " << pfp4.E() << std::endl;
